...
const exif_1 = require("../services/exif");
const orchestrator_1 = require("../agents/orchestrator");
const router = (0, express_1.Router)();
const upload = (0, multer_1.default)({ limits: { fileSize: 20 * 1024 * 1024 } }); // 20MB
 ...
 */
router.post('/upload', auth_1.authMiddleware, upload.single('photo'), async (req, res) => {
    try {
        const file = req.file;
        ...
        // 1. EXIF 硬件级元数据解析
        const exif = fileBuffer.length > 0 ? exif_1.exifService.parse(fileBuffer) : {};
        // 2. 上传至对象存储
        const uploadedUrl = await oss_1.storageService.uploadFile(fileBuffer, originalname);
        // 3. 触发多智能体流水线 (VisionAgent + LocationAgent + StoryAgent)
        const pipelineResult = await orchestrator_1.orchestrator.processMemoryPipeline({
 ...
 * 点赞记忆卡片
 */
router.post('/:id/like', auth_1.authMiddleware, (req, res) => {
    const { id } = req.params;
