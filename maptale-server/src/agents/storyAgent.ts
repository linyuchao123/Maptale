...
import type { VisionAnalysisResult, LocationResolutionResult, StoryCreationResult } from './types'
...
请注意：只返回纯 JSON，严禁包裹 markdown 代码块或附带其他文字。`
        ...
        if (content) {
          ...
          return JSON.parse(cleaned) as StoryCreationResult
        }
      } catch (err) {
        console.warn('[StoryAgent] DeepSeek API 调用异常，使用高水准文案引擎生成:', (err as Error).message)
      }
    }
