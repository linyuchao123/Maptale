...
// 中国各省份别名与地标关键词字典映射
const PROVINCE_MAPPING: Record<string, { id: string; name: string; keywords: string[] }> = {
  yunnan: {
    id: 'yunnan',
    name: '云南',
...
export class LocationAgent {
  private amapKey: string | undefined
  ...
  async resolve(params: {
    ...
    if (lat && lng && this.amapKey) {
      try {
        ...
        console.warn('[LocationAgent] 高德逆地理编码出错，切换到文本规则匹配:', (err as Error).message)
      }
    }
  ...
  private matchProvince(text: string): { id: string; name: string } {
  ...
  private matchCoordinates(lat: number, lng: number): { id: string; name: string } | null {
    if (lat >= 21.0 && lat <= 29.5 && lng >= 97.0 && lng <= 106.5) return { id: 'yunnan', name: '云南' }
    if (lat >= 26.0 && lat <= 34.5 && lng >= 97.0 && lng <= 108.5) return { id: 'sichuan', name: '四川' }
    if (lat >= 39.4 && lat <= 41.1 && lng >= 115.4 && lng <= 117.5) return { id: 'beijing', name: '北京' }
    if (lat >= 26.8 && lat <= 36.5 && lng >= 78.4 && lng <= 99.1)  return { id: 'xizang', name: '西藏' }
    if (lat >= 27.0 && lat <= 31.5 && lng >= 118.0 && lng <= 123.0) return { id: 'zhejiang', name: '浙江' }
    if (lat >= 20.0 && lat <= 25.5 && lng >= 109.5 && lng <= 117.5) return { id: 'guangdong', name: '广东' }
    return null
  }
}
