...
import type { ProcessedMemoryResult } from './types'
...
export class OrchestratorAgent {
  ...
  async processMemoryPipeline(params: {
    ...
    const [visionResult, locationResult] = await Promise.all([
      visionAgent.analyze(imageUrl).catch(err => {
      ...
      locationAgent.resolve({ lat, lng, hintText }).catch(err => {
    ...
    const storyResult = await storyAgent.createStory({
      vision: visionResult,
